...
select
    {{ dbt_utils.star(
        from=ref('fact_rental'),
        relation_alias='fact_rental',
        except=["rental_key","customer_key","customer_address_key","film_key"]
    ) }},

    {{ dbt_utils.star(
        from=ref('dim_customer'),
        relation_alias='dim_customer',
        prefix='customer_',
        except=["customer_key","last_update"]
    ) }},

    {{ dbt_utils.star(
        from=ref('dim_address'),
        relation_alias='dim_address',
        prefix='address_',
        except=["address_key"]
    ) }},
    ...
    {{ dbt_utils.star(
        from=ref('dim_date'),
        relation_alias='dim_date_rental_date',
        prefix='rental_date_',
        except=["date_day"]
    ) }},
    ...
    {{ dbt_utils.star(
        from=ref('dim_date'),
        relation_alias='dim_date_return_date',
        prefix='return_date_',
        except=["date_day"]
    ) }}
