...
The transformation layer is built using **warehouse-portable dbt models**, tested on both **Databricks and Snowflake**, allowing the pipeline to run unchanged across platforms by simply switching execution targets.
...
The batch dashboard presents historical user behaviour and content performance trends, enabling consistent reporting and long-term, strategic analysis. Data comes from the one big table (OBT) serving layer of Databricks (or Snowflake). For more details, refer to [page](../../04-data-consumption/batch/README.md).
...
2. <img src="https://cdn.simpleicons.org/airbyte/615EFF" width="24" height="24">**Airbyte**: extracts data from a Postgres RDS source **(A)** and performs a incremental append laods into **Databricks (or Snowflake) Bronze** (raw) tables.
3. <img src="https://dagster-website.vercel.app/images/brand/logos/dagster-primary-mark.png" width="24" height="24" alt="Dagster">**Dagster+**: orchestrates dbt Core transformations and data model builds. Builds Silver “current-state” models (mostly SCD1 dedup) and maintains Customer SCD2 history using dbt snapshots (dvd_customer_scd2) plus a current view (dvd_customer).
4. <img src="https://www.iconarchive.com/download/i149529/simpleicons-team/simple/dbt.512.png" width="24" height="24" alt="dbt"> **DBT**: performs transformations in **Databricks (or Snowflake)** from **Bronze → Silver**.
5. <img src="https://www.iconarchive.com/download/i149529/simpleicons-team/simple/dbt.512.png" width="24" height="24" alt="dbt" > **DBT**: performs transformations in **Databricks (or Snowflake)** from **Silver → Gold**, producing analytics-ready models. Builds incremental MERGE models for mutable/high-value datasets (e.g. fact_rental, dim_customer).
...
Implemented bronze → silver → gold data modelling on **Databricks (and Snowflake)** database.

![DB Structure Databricks](../../02-data-transformation/batch/images/batch-db-databricks.png)

![DB Structure Snowflake](../../02-data-transformation/batch/images/batch-db-snowflake.png)
...
- **History & Time Travel (Databricks and Snowflake):**
...
- **dbt materializations (Databricks + Delta or Snowflake):**
...
- Access to:
  ...
  - <img src="https://cdn.simpleicons.org/databricks/FF3621" width="24" height="24"> Databricks or Snowflake workspace
...
3. **Configure components:**
   - **Airbyte:** Set up connection from PostgreSQL to Databricks (or Snowflake) ([Airbyte Setup](../../01-data-ingestion/batch/README.md))
